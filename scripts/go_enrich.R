...
fisher.stat <- new("classicCount", testStatistic = GOFisherTest, 
                   name = "Fisher test")
resultFisher <- getSigGroups(GOdata, fisher.stat)
weight.stat <- new("weightCount", testStatistic = GOFisherTest, 
                   name = "Fisher test with weight algorithm", sigRatio = 'ratio')
...
allRes <- GenTable(GOdata, classic = resultFisher, weight = resultWeight, 
                   orderBy = "weight", ranksOf = "classic", topNodes = 200)
